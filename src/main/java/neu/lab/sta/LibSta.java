...
public class LibSta extends Sta{
	...
	public void calPassInfo() {
		...
		for (String acsMthd : this.acsJarMes) {
			...
			Set<String> outMthds = (Set<String>) SysInfo.books.get(acsMthd).getRecords();
		...
		for (String hostMthd : SysInfo.hostMthds) {
			@SuppressWarnings("unchecked")
			Set<String> outMthds = (Set<String>) SysInfo.books.get(hostMthd).getRecords();
			for (String outMthd : outMthds) {
