...
import neu.lab.sta.HostSta;
import neu.lab.sta.LibSta;
...
public class JarStaPanel extends JPanel {
	...
	private void initTableP(HostSta hostSta, Collection<LibSta> jarStas) {
		tableP = new JScrollPane();
		Object[][] rows = new Object[jarStas.size() + 1][];

		rows[0] = hostSta.staRow(StaTableModel.tHead);
		int i = 1;
		for (LibSta jarSta : jarStas) {
			rows[i] = jarSta.staRow(StaTableModel.tHead);
			i++;
		}
	...
	private JCheckBox getMthdCkBox(String text) {
		...
		checkBox.addMouseListener(new MouseInOutListener("显示方法级别的统计指标"));
	...
	private JCheckBox getClsCkBox(String text) {
		JCheckBox checkBox = new JCheckBox(text);
		checkBox.addMouseListener(new MouseInOutListener("显示类级别的统计指标"));
		checkBox.addItemListener(new ItemListener() {
			@Override
			public void itemStateChanged(ItemEvent e) {
				JCheckBox box = (JCheckBox) e.getItem();
